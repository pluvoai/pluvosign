...
// Runtime-configurable app name. Set per deployment via the `appName`
// env var (see entrypoint.sh / .env.prod). Falls back to "PluvoSign".
export function getAppName() {
  const v = getEnv()?.appName;
  return v && v.trim() ? v : "PluvoSign";
}
const appName = getAppName();

export const defaultMailBody = `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#eef1f4;"><tr><td align="center" style="padding:32px 16px;">
...
<tr><td align="center" style="padding:40px 40px 20px 40px;"><img src="https://sign.pluvoai.com/pluvosign-logo.png" alt="${appName}" height="48" style="display:block;height:48px;width:auto;"></td></tr>
...
<p style="margin:0 0 14px 0;font-size:15px;line-height:1.65;color:#42505f;">Hi {{receiver_name}},</p>
<p style="margin:0 0 26px 0;font-size:15px;line-height:1.65;color:#42505f;"><strong style="color:#1f2a37;">{{sender_name}}</strong> has requested your signature on <strong style="color:#1f2a37;">{{document_title}}</strong>. Please review the document and sign at your earliest convenience.</p>
</td></tr>
...
<a href="{{signing_url}}" target="_blank" rel="noopener noreferrer" style="display:inline-block;padding:15px 46px;font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;font-size:15px;font-weight:600;color:#ffffff;text-decoration:none;">Review &amp; Sign</a>
</td></tr></table>
</td></tr>
<tr><td align="center" style="padding:6px 40px 34px 40px;font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;font-size:12px;line-height:1.6;color:#9aa5b1;">Button not working? Copy and paste this link into your browser:<br><span style="color:#3E8FCB;word-break:break-all;">{{signing_url}}</span></td></tr>
...
<p style="margin:0 0 8px 0;font-size:12px;line-height:1.6;color:#9aa5b1;">If you have any questions about this document, please contact the sender directly.</p>
...
export const embedWidgetsToDoc = async (
  ...
  if (!hasError) {
    ...
    return {
      error:
        "This PDF could not be processed. Please contact reachus@pluvoai.com for assistance."
    };
  }
};
...
export const getAppLogo = async () => {
  ...
  try {
    ...
    if (tenant) {
      ...
      localStorage.setItem("appname", getAppName());
...
export const mailTemplate = (param) => {
  const appName = getAppName();
  const logoUrl = "https://sign.pluvoai.com/pluvosign-logo.png";
  ...
  const noteRow = param.note
    ...
    : "";
  const body = `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#eef1f4;"><tr><td align="center" style="padding:32px 16px;">
<table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background-color:#ffffff;border:1px solid #e3e7ec;border-radius:12px;">
...
export const sendEmailToSigners = async (
  ...
  for (let i = 0; i < signerMail.length; i++) {
    try {
      ...
      // Surface demo-mode rejections clearly — the generic "mail-failed" UI
      // tells users to "try again", which is misleading when the wrapper has
      // deliberately blocked the send.
      const apiMessage = error?.response?.data?.error || "";
      if (apiMessage.startsWith("Demo mode:")) {
