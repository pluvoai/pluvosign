...
const eSignName = process.env.appName || 'PluvoSign';
const eSigncontact = 'reachus@pluvoai.com';
...
async function sendNotifyMail(doc, signUser, mailProvider, publicUrl) {
  try {
    ...
    // Use the tenant's own Domain so multi-tenant deployments don't leak the
    // server's PUBLIC_URL into emails. Falls back when tenant has no Domain set.
    const tenantDomain = doc?.ExtUserPtr?.TenantId?.Domain;
    const baseUrl = tenantDomain
      ? `https://${tenantDomain}`
      : process.env.PUBLIC_URL || 'https://sign.pluvoai.com';
    ...
    if (remainingSign > 1 && doc?.NotifyOnSignatures) {
      ...
      const body = `<!DOCTYPE html><html><head><meta charset="utf-8"></head><body style="margin:0;padding:0;background-color:#eef1f4;">
...
<tr><td align="center" style="padding:40px 40px 20px 40px;"><img src="${logoUrl}" alt="${TenantAppName}" height="48" style="display:block;height:48px;width:auto;"></td></tr>
...
<h1 style="margin:0 0 18px 0;font-size:23px;line-height:1.3;color:#1f2a37;font-weight:600;">Document signed</h1>
<p style="margin:0 0 14px 0;font-size:15px;line-height:1.65;color:#42505f;">Hi ${creatorName},</p>
<p style="margin:0 0 26px 0;font-size:15px;line-height:1.65;color:#42505f;"><strong style="color:#1f2a37;">${signerName}</strong> (${signerEmail}) has signed <strong style="color:#1f2a37;">${pdfName}</strong>.</p>
</td></tr>
...
<a href="${viewDocUrl}" target="_blank" style="display:inline-block;padding:15px 46px;font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;font-size:15px;font-weight:600;color:#ffffff;text-decoration:none;">View Document</a>
</td></tr></table>
</td></tr>
<tr><td style="padding:28px 40px 0 40px;"><div style="border-top:1px solid #e7ebef;font-size:0;line-height:0;">&nbsp;</div></td></tr>
<tr><td style="padding:22px 40px 34px 40px;font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;">
<p style="margin:0;font-size:12px;line-height:1.6;color:#9aa5b1;">This is an automated message from ${TenantAppName}. For any questions about this document, please contact the sender directly.</p>
</td></tr>
</table>
...
async function sendCompletedMail(obj) {
  ...
  let body = `<!DOCTYPE html><html><head><meta charset="utf-8"></head><body style="margin:0;padding:0;background-color:#eef1f4;">
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#eef1f4;"><tr><td align="center" style="padding:32px 16px;">
<table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background-color:#ffffff;border:1px solid #e3e7ec;border-radius:12px;">
<tr><td align="center" style="padding:40px 40px 20px 40px;"><img src="${logoUrl}" alt="${TenantAppName}" height="48" style="display:block;height:48px;width:auto;"></td></tr>
<tr><td style="padding:0 40px;"><div style="height:3px;background-color:#3E8FCB;border-radius:2px;font-size:0;line-height:0;">&nbsp;</div></td></tr>
<tr><td style="padding:32px 40px 0 40px;font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;">
<h1 style="margin:0 0 18px 0;font-size:23px;line-height:1.3;color:#1f2a37;font-weight:600;">Document completed</h1>
<p style="margin:0 0 26px 0;font-size:15px;line-height:1.65;color:#42505f;">All parties have signed <strong style="color:#1f2a37;">"${pdfName}"</strong>. A copy of the completed document is attached to this email for your records.</p>
</td></tr>
<tr><td style="padding:4px 40px 0 40px;"><div style="border-top:1px solid #e7ebef;font-size:0;line-height:0;">&nbsp;</div></td></tr>
<tr><td style="padding:22px 40px 34px 40px;font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;">
<p style="margin:0;font-size:12px;line-height:1.6;color:#9aa5b1;">This is an automated message from ${TenantAppName}. For any questions about this document, please contact the sender directly.</p>
</td></tr>
</table>
